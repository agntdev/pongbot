 ...
 */
export async function runSpecs(makeBot, specs) {
    const results = [];
...
export function formatSuiteResult(suite) {
    const header = `${suite.passed}/${suite.total} specs passed${suite.failed > 0 ? ` (${suite.failed} failed)` : ""}`;
    const lines = [header];
    for (const r of suite.results) {
        ...
        if (!r.ok) {
            r.steps.forEach((st, i) => {
                if (!st.ok)
                    lines.push(`    step ${i + 1}: ${st.failures.join("; ")}`);
            });
        }
    }
    return lines.join("\n");
...
export function parseBotSpec(obj) {
    ...
    if (!Array.isArray(o.steps))
        throw new Error("spec.steps must be an array");
    o.steps.forEach((step, i) => {
        if (typeof step !== "object" || step === null)
            throw new Error(`step ${i} must be an object`);
        const s = step;
        if (typeof s.send !== "object" || s.send === null)
            throw new Error(`step ${i} missing 'send'`);
        if (!Array.isArray(s.expect))
            throw new Error(`step ${i} 'expect' must be an array`);
    });
    return obj;
}
/** Validate an array of specs (e.g. a parsed specs.json). */
export function parseBotSpecs(arr) {
