...
function commandOfSend(send) {
    if (!("text" in send) || typeof send.text !== "string")
        return null;
...
function stepIsMeaningful(step) {
...
export function commandsInSpec(spec) {
    const out = new Set();
    for (const step of spec.steps) {
        if (!stepIsMeaningful(step))
            continue;
...
export function commandsInSpecs(specs) {
    const out = new Set();
    for (const s of specs)
        for (const c of commandsInSpec(s))
            out.add(c);
    return out;
}
...
export function normalizeDeclaredCommands(declared) {
    const seen = new Set();
 ...
 */
export function computeCoverage(specs, declared) {
    const decl = normalizeDeclaredCommands(declared);
    const exercised = commandsInSpecs(specs);
    const covered = [];
    const missing = [];
    for (const c of decl)
        (exercised.has(c) ? covered : missing).push(c);
