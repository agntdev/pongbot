# E1T3 — Set up SQLite persistence for the global counter

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 PONG

Create a SQLite database at ./data/pings.sqlite with a table `counter` containing a single row ('pings', 0). Ensure that the /ping command increments the counter in a single SQL transaction to avoid race conditions.
