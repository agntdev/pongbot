# fix-b9c9cb8c3ff25992 — /count command handler references undefined variables — will crash

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 PONG

`src/bot.ts:39` references `pingStore.read(PING_COUNT_KEY)` but neither `pingStore` nor `PING_COUNT_KEY` is imported or defined anywhere in the file. This causes a `ReferenceError` at runtime whenever a user sends `/count`. The callback handler for `cmd:count` (line 57-61) correctly uses `getCount()` from `./persistence.js` — the command handler was not updated to match. Fix: replace lines 39-40 with `const count = await getCount();`.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-b9c9cb8c3ff25992.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-b9c9cb8c3ff25992.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
