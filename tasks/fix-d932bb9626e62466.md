# fix-d932bb9626e62466 — Conflicting /start test spec — gate will fail

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 PONG

`tests/specs/start.json` expects `/start` to reply with `"Welcome! I am ready to help."`, but `src/bot.ts:44-49` replies with `"Hi — I'm PongBot! Send /ping to get a pong 🏓 and increment the global counter."`. The Tests-gate globs all `tests/specs/*.json`, so the `start.json` spec will fail against the current bot. Additionally, `tests/commands/start.json` and `tests/commands/T02.json` both declare `["/start"]`, making a duplicate command manifest. Fix: either update `start.json` to match the current bot response, or remove the stale spec files if they belong to a superseded task.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-d932bb9626e62466.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-d932bb9626e62466.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
