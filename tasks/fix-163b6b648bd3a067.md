# fix-163b6b648bd3a067 — /help command omits /count from command listing

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 PONG

`src/bot.ts:64-69` lists `/start`, `/ping`, and `/help` in the help message but omits `/count` — even though `/count` is a core command per `docs/spec.md`. Users cannot discover `/count` through the help command. The test spec `tests/specs/T03.json` encodes this omission (so the test passes), but the help listing is incomplete.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-163b6b648bd3a067.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-163b6b648bd3a067.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
